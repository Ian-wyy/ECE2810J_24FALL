...
struct node {
  ...
  node(const Point2D &k)
      : key(k), left_subtree(nullptr), right_subtree(nullptr) {
    domain.push_back({std::numeric_limits<int>::min(),
                      std::numeric_limits<int>::max()}); // X domain
    domain.push_back({std::numeric_limits<int>::min(),
                      std::numeric_limits<int>::max()}); // Y domain
  }
};
...
private:
  ...
  static node *buildTree(typename std::vector<Point2D>::iterator start,
                         typename std::vector<Point2D>::iterator end,
                         int depth) {
    if (start >= end)
      return nullptr;
  ...
  static void nearestNeighborSearch(node *currentNode,
                                    const Point2D &queryPoint, int depth,
                                    Point2D &bestPoint, double &bestDist) {
    if (!currentNode)
      return;
    ...
    if ((axis == 0 && queryPoint.x < currentNode->key.x) ||
        (axis == 1 && queryPoint.y < currentNode->key.y)) {
    ...
    if (oppositeBranch != nullptr &&
        ((axis == 0 && abs(queryPoint.x - currentNode->key.x) < bestDist) ||
         (axis == 1 && abs(queryPoint.y - currentNode->key.y) < bestDist))) {
      nearestNeighborSearch(oppositeBranch, queryPoint, depth + 1, bestPoint,
                            bestDist);
    }
  }
...
int main() {
  ...
  std::vector<Point2D> points = {{2, 3}, {5, 4}, {9, 6},
                                 {4, 7}, {8, 1}, {7, 2}};
  ...
  std::cout << "The nearest neighbor is at: (" << nearest.x << ", " << nearest.y
            << ")" << std::endl;
