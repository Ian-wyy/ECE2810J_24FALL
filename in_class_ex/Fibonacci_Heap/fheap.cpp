...
public:
  ...
  FibonacciNode *getMinNode() { return minNode; }
  ...
  void extractMin() {
    ...
    do{
        FibonacciNode* right_child = child->right;
        child = right_child->right;
        right_child->parent = NULL;
        right_child->marked = false;
        right_child->left = min->left;
        right_child->right = min->right;
        min->left->right = right_child;
        min->right->left  = right_child;
    }while()
    }
  }
...
int main() {
  ...
  return 0; // Stopped here for the Exercise 1; Remove this line after Exercise
            // 3;

  fibHeap.extractMin();
  std::cout << "Minimum after extraction: " << fibHeap.findMin() << std::endl;

  return 0; // Stopped here for the Exercise 2; Remove this line after Exercise
            // 2;

  // Decrease the key of a node
  FibonacciNode *nodeToDecrease =
      fibHeap.getMinNode()->right; // Decrease the second smallest node
  ...
  fibHeap.insert(10);
  fibHeap.insert(3);
  FibonacciNode *nodeToDecrease2 =
      fibHeap.getMinNode()->right; // Decrease the second smallest node again
  fibHeap.decreaseKey(nodeToDecrease2, 2);
  std::cout << "Minimum after another decreaseKey: " << fibHeap.findMin()
            << std::endl;

  // Add nodes with equal keys and test decreaseKey
  fibHeap.insert(2); // Insert a node with the same key as an existing node
  std::cout << "Minimum before decreaseKey with equal keys: "
            << fibHeap.findMin() << std::endl;
  FibonacciNode *nodeToDecrease3 =
      fibHeap.getMinNode()
          ->right; // Decrease the second smallest node with equal keys
  fibHeap.decreaseKey(nodeToDecrease3, 1);
  std::cout << "Minimum after decreaseKey with equal keys: "
            << fibHeap.findMin() << std::endl;
